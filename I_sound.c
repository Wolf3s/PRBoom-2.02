...
HRESULT CreateSecondaryBuffer(LPDIRECTSOUNDBUFFER *lplpDsb, int size)
{
    PCMWAVEFORMAT pcmwf;
...
// proff 07/04/98: Added for CYGWIN32 compatibility
#ifdef DIRECTX
  error = DirectSoundCreate(NULL,&lpDS,NULL);
  ...
  lprintf(LO_INFO,"created DirectSound\n");
...
//  SafemciSendString("close doommusic");
  if    //jff 02/08/98 add native midi support
    ...
    {
    boom_printf("Error loading midi: %d",err);
    return -1;
    }
  switch (used_mus_card)
  {
  ...
  case MCI_MIDI:
    ...
    M_WriteFile("boom.mid",mid,midlen);
    SafemciSendString("open boom.mid alias doommusic");
    free(mid);
    break;
#endif
#ifdef STREAM_MIDI
...
#include "i_sound.h"
#include "w_wad.h"
#include "g_game.h"     //jff 1/21/98 added to use boom_printf in I_RegisterSong
#include "d_main.h"
