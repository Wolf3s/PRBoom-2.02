  ...
  if (paused)
    {
      ...
      V_DrawPatchStretched(viewwindowx+((SCREENWIDTH-320)/2)+(scaledviewwidth-68)/2,
                        (y*200)/SCREENHEIGHT,0,W_CacheLumpName ("M_PAUSE", PU_CACHE));
...
// proff/nicolas 09/14/98 -- now stretchs bitmaps to fullscreen!
  V_DrawPatchStretched (0, 0, 0, W_CacheLumpName(pagename, PU_CACHE));
...
// proff 07/04/98: Changed from _MSC_VER to _WIN32 for compatibility to CYGWIN32
#ifdef _WIN32
  sprintf(basedefault,"boom.cfg", D_DoomExeDir());  // proff
#else //_MSC_VER
...
// proff 07/04/98: Changed for compatibility to MINGW32
...
// proff 07/04/98: Changed from _MSC_VER to _WIN32 for compatibility to CYGWIN32
#ifdef _WIN32
      strcpy(basedefault, "c:/doomdata/boom.cfg");       // proff
#else
