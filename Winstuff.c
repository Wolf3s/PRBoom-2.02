...
HWND ghWnd;
char szAppName[] = "BoomWinClass";
char szTitlePrefix[] = "Boom 2.02 - ";
char szConTitlePrefix[] = "Boom Console 2.02 - ";
...
char szConName[] = "BoomConWinClass";
  ...
  case WM_PAINT:
	  if (dc = BeginPaint (con_hWnd, &paint))
    {
		  if (Last)
      {
        ...
        {
				  TextOutA (dc, 0, line * OemHeight, row + 2, row[1]);
				  row += 80 + 2;
			  }
		  }
...
void I_PrintStr (int xp, const char *cp, int count, BOOL scroll) {
	...
	if (count)
  {
    conDC=GetDC(con_hWnd);
		TextOutA (conDC, xp * OemWidth, ConHeight - OemHeight, cp, count);
...
// proff 08/17/98: Changed for high-res
    if (i=M_CheckParm("-width"))
      SCREENWIDTH = atol(myargv[i+1]);
    if (i=M_CheckParm("-height"))
      SCREENHEIGHT = atol(myargv[i+1]);
    if (SCREENWIDTH<320)
      SCREENWIDTH=320;
    if (SCREENWIDTH>1920)
      SCREENWIDTH=1920;
...
// proff 11/06/98: Added setting of console title
  ...
  *pEnd = 0;
  ...
  SetWindowTextA(con_hWnd,szConTitle);
}
...
void DDrawFullscreen(int fullscreen)
{
  ...
  if (fullscreen)
  {
    ...
    if (error != DD_OK)
    {
      ...
      boom_printf("DDrawMode %ix%i",BestWidth,BestHeight);
