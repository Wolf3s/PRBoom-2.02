...
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched(x, y, 0, hu_font[c]);
...
#ifdef _WIN32
    {"sound_card", &snd_card, 1,
     0,1,0,ss_none,    "[0-1(1)] 0=No Sound, 1=DirectSound"}, 

    {"music_card", &mus_card, 2,
     0,2,0,ss_none,    "[0-2(2)] used to select MIDI transfer type. 0=No Music, 1=MCI-MIDI, 2=Stream-MIDI"},  

// proff: Moved snd_channels to top and reduced default to 8
    {"snd_channels",&numChannels, 8,
     1,UL,0,ss_none,   "[1-?(8)] number of audio events simultaneously"},  // killough

// proff 08/17/98: Added the following to allow change without recompile
    {"snd_frequency",&snd_freq, 11025,
     11025,44100,0,ss_none,   "[11025-44100(11025)] sample frequency"},  // proff

    {"snd_bits",&snd_bits, 8,
     8,16,0,ss_none,   "[8/16(8)] bits per sample"},  // proff

    {"snd_stereo",&snd_stereo, 1,
     0,1,0,ss_none,   "[0/1(1)] use stereo"},  // proff

    {"vid_fullscreen",&vidFullScreen, 0,
     0,1,0,ss_none,   "[0/1(0)] 1 to start fullscreen"},  // proff
#endif //_WIN32
...
//#ifdef NORMALUNIX // proff 6/17/98
...
// proff 08/17/98: Added backward to mousebuttons
    {"mouseb_backward",&mousebbackward,-1,
     -1,2,0,ss_keys,    "[-1-2(-1)] mouse button number to use for forward motion"},
...
#ifndef _WIN32
// proff: Moved snd_channels to top
...
void M_LoadDefaults (void)
  ...
  for (i = 0 ; i < numdefaults ; i++)
    ...
    *defaults[i].location = defaults[i].defaultvalue;
    ...
    if (defaults[i].isstr)
      *defaults[i].location = (intptr_t)strdup((char *)*defaults[i].location);
    }
...
void M_ScreenShot (void)
  ...
  if (access(".",2))
    {
    boom_printf ("M_ScreenShot: Couldn't create a PCX"); // killough 4/18/98
    return;
    }
