...
// Externalized to s_GAMMALVLx strings in d_deh.h
//
//char gammamsg[5][26] =
//{
//    GAMMALVL0,
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (94,2,0,W_CacheLumpName("M_DOOM",PU_CACHE));
...
//    V_DrawPatchDirect (0,0,0,W_CacheLumpName("HELP",PU_CACHE));
    ...
    case registered:
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched (0,0,0,W_CacheLumpName("HELP2",PU_CACHE));
//      V_DrawPatchDirect (0,0,0,W_CacheLumpName("HELP2",PU_CACHE));
      break;
    case retail:   // killough 2/21/98: Fix Ultimate Doom help screen:
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched (0,0,0,W_CacheLumpName("CREDIT",PU_CACHE));
...
void M_DrawEpisode(void)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (54,38,0,W_CacheLumpName("M_EPISOD",PU_CACHE));
...
void M_DrawNewGame(void)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (96,14,0,W_CacheLumpName("M_NEWG",PU_CACHE));
  V_DrawPatchStretched (54,38,0,W_CacheLumpName("M_SKILL",PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (72,LOADGRAPHIC_Y,0,W_CacheLumpName("M_LOADG",PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (x-8,y+7,0,W_CacheLumpName("M_LSLEFT",PU_CACHE));
//  V_DrawPatchDirect (x-8,y+7,0,W_CacheLumpName("M_LSLEFT",PU_CACHE));
  
  for (i = 0 ; i < 24 ; i++)
    {
// proff/nicolas 09/20/98 -- changed for hi-res
    V_DrawPatchStretched (x,y+7,0,W_CacheLumpName("M_LSCNTR",PU_CACHE));
//    V_DrawPatchDirect (x,y+7,0,W_CacheLumpName("M_LSCNTR",PU_CACHE));
    x += 8;
    }

// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (x,y+7,0,W_CacheLumpName("M_LSRGHT",PU_CACHE));
...
void M_ForcedLoadGame(const char *msg)
  {
  M_StartMessage(strdup(msg), M_VerifyForcedLoadGame, true);
  }
...
void M_DrawSave(void)
  ...
  //jff 3/15/98 use symbolic load position
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (72,LOADGRAPHIC_Y,0,W_CacheLumpName("M_SAVEG",PU_CACHE));
...
void M_DrawOptions(void)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (108,15,0,W_CacheLumpName("M_OPTTTL",PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (OptionsDef.x + 120,OptionsDef.y+LINEHEIGHT*messages,0,
...
void M_DrawSound(void)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (60,38,0,W_CacheLumpName("M_SVOL",PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (60,38,0,W_CacheLumpName("M_MSENS",PU_CACHE));
...
void M_DrawSetup(void)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched(124,15,0,W_CacheLumpName("M_SETUP",PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched(x,y,0,W_CacheLumpName(ResetButtonName[whichSkull],PU_CACHE));
//      V_DrawPatchDirect(x,y,0,W_CacheLumpName(ResetButtonName[whichSkull],PU_CACHE));
    else
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched(x,y,0,W_CacheLumpName(ResetButtonName[0],PU_CACHE));
...
// proff/nicolas 09/20/98 -- changed for hi-res
      V_DrawPatchStretched (x+1,y,0,W_CacheLumpName("M_PALNO",PU_CACHE));
...
// proff 08/17/98: Added backward to mousebuttons
  {"BACKWARD"    ,S_KEY       ,m_scrn,KB_X,KB_Y+ 2*8,&key_down          ,&mousebbackward,0,0,0,0,0},
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (84,2,0,W_CacheLumpName("M_KEYBND",PU_CACHE));
...
void M_DrawWeapons(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (109,2,0,W_CacheLumpName("M_WEAP",PU_CACHE));
...
void M_DrawStatusHUD(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (59,2,0,W_CacheLumpName("M_STAT",PU_CACHE));
...
setup_menu_t auto_settings2[] =  // 2nd AutoMap Settings screen
{
  ...
  {"player 1 arrow"                 ,S_COLOR ,m_null,AU_X,AU_Y+ 8*8,0,0,0,&mapcolor_plyr[0],0,0,0},
  {"player 2 arrow"                 ,S_COLOR ,m_null,AU_X,AU_Y+ 9*8,0,0,0,&mapcolor_plyr[1],0,0,0},
  {"player 3 arrow"                 ,S_COLOR ,m_null,AU_X,AU_Y+10*8,0,0,0,&mapcolor_plyr[2],0,0,0},
  {"player 4 arrow"                 ,S_COLOR ,m_null,AU_X,AU_Y+11*8,0,0,0,&mapcolor_plyr[3],0,0,0},
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (COLORPALXORIG-5,COLORPALYORIG-5,0,W_CacheLumpName("M_COLORS",PU_CACHE));
...
void M_DrawAutoMap(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (109,2,0,W_CacheLumpName("M_AUTO",PU_CACHE));
...
void M_DrawEnemy(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (114,2,0,W_CacheLumpName("M_ENEM",PU_CACHE));
...
void M_DrawMessages(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (103,2,0,W_CacheLumpName("M_MESS",PU_CACHE));
...
void M_DrawChatStrings(void)
  ...
  M_DrawBackground("FLOOR4_6"); // Draw background
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (83,2,0,W_CacheLumpName("M_CHAT",PU_CACHE));
...
void M_InitExtendedHelp(void)
  ...
  {
  int index,i;
// proff 07/04/98: Changed for CYGWIN32 compatibility, because it caused a SEGV
//  char* namebfr = "HELPnn";
  char* namebfr;

// proff 07/04/98: Added for CYGWIN32 compatibility
  namebfr=malloc(7);
  strcpy(namebfr,"HELPnn");
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched(0,0,0,W_CacheLumpName(namebfr,PU_CACHE));
...
// proff 08/17/98: Added backward to mousebuttons
  {"BACKWARD"    ,(S_SKIP|S_KEY),m_null,KT_X3,KT_Y3+ 2*8,&key_down        ,&mousebbackward,0,0,0,0,0},
...
// proff/nicolas 09/20/98 -- changed for hi-res
    V_DrawPatchTransStretched(cx,cy,0,hu_font[c],colrngs[color],0);
          ...
          if (ch == key_menu_backspace) // backspace and DEL
            ...
            if (chat_string_buffer[chat_index] != 0)
              chat_index++;
               ...
               (ch == key_menu_escape))
            {
            ptr1->m_var1 = chat_string_buffer;
            M_SelectDone(ptr1);         // phares 4/17/98
            }
...
void M_Drawer (void)
  ...
  if (messageToPrint)
    ...
    while(*(messageString+start))
      {
// proff 08/15/98: Changed this function a little bit, because it didn't work always

//      for (i = 0;/*(size_t)i < strlen(messageString+start)*/;i++)
      for (i = 0;;i++)
//        if (*(messageString+start+i) == '\n')
        if ((*(messageString+start+i) == '\n') || (*(messageString+start+i) == 0))
        {
          memset(string,0,40);
...
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched(x + SKULLXOFF,
...
void M_DrawThermo(int x,int y,int thermWidth,int thermDot )
  ...
  xx = x;
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (xx,y,0,W_CacheLumpName("M_THERML",PU_CACHE));
//  V_DrawPatchDirect (xx,y,0,W_CacheLumpName("M_THERML",PU_CACHE));
  xx += 8;
  for (i=0;i<thermWidth;i++)
    {
// proff/nicolas 09/20/98 -- changed for hi-res
    V_DrawPatchStretched (xx,y,0,W_CacheLumpName("M_THERMM",PU_CACHE));
//    V_DrawPatchDirect (xx,y,0,W_CacheLumpName("M_THERMM",PU_CACHE));
    xx += 8;
    }
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (xx,y,0,W_CacheLumpName("M_THERMR",PU_CACHE));
//  V_DrawPatchDirect (xx,y,0,W_CacheLumpName("M_THERMR",PU_CACHE));

// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched ((x+8) + thermDot*8,y,
...
void M_DrawEmptyCell (menu_t* menu,int item)
  {
// proff/nicolas 09/20/98 -- changed for hi-res
  V_DrawPatchStretched (menu->x - 10,menu->y+item*LINEHEIGHT - 1, 0,
